...
export const LiveActivityLogPanel: React.FC<LiveActivityLogPanelProps> = ({ onClose, isSidePanel = false }) => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = logger.subscribe((newEntry) => {
      ...
      // If in newest_top mode, scroll to top on new entry
      if (sortOrder === 'newest_top' && logsContainerRef.current) {
  ...
  // Complete Wipe of all old localStorage and history
  const handleWipeAllAndStartFresh = () => {
    if (confirm('Wipe all stored telemetry and start with a 100% clean session?')) {
  ...
  // Sorting: newest_top has index 0 (newest) at top; oldest_top inverts it
  const displayLogs = sortOrder === 'newest_top'
    ? filteredLogs
    ...
    <div className={`flex flex-col h-full bg-slate-950/95 backdrop-blur-md border-l border-slate-800 text-slate-100 ${isSidePanel ? 'w-full shadow-2xl' : 'rounded-3xl border shadow-2xl overflow-hidden'}`}>
      ...
      <div className="px-4 py-3 border-b border-slate-800/90 flex items-center justify-between bg-slate-900/70 shrink-0">
        <div className="flex items-center space-x-2.5">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-400 font-mono">
              Active Session: {logger.getSessionId()}
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center space-x-1.5">
          ...
          <button
            type="button"
            onClick={() => setSortOrder(prev => prev === 'newest_top' ? 'oldest_top' : 'newest_top')}
            className="px-2 py-1 rounded-xl bg-slate-800 hover:bg-slate-700 border border-slate-700 text-[10px] font-mono text-teal-300 transition cursor-pointer"
            title="Toggle between Newest-First (Top) and Oldest-First (Terminal)"
          >
            {sortOrder === 'newest_top' ? '⚡ Newest on Top' : '💻 Oldest on Top'}
          </button>
          ...
          <button
            ...
            className="p-1.5 rounded-xl bg-slate-800 hover:bg-rose-900/40 border border-slate-700 hover:border-rose-500/40 text-slate-400 hover:text-rose-300 text-xs transition cursor-pointer"
            title="Wipe all historical logs and localStorage data"
          >
      ...
      <div ref={logsContainerRef} className="flex-1 overflow-y-auto p-3 space-y-2 font-mono text-xs custom-scrollbar">
          ...
          <>
            {displayLogs.length === 0 ? (
              ...
              displayLogs.map((entry, idx) => {
                const isLatest = (sortOrder === 'newest_top' && idx === 0) || (sortOrder === 'oldest_top' && idx === displayLogs.length - 1);
                ...
                return (
                  <div
                    ...
                    className={`p-2.5 rounded-xl border ${style.border} ${style.bg} transition-all duration-150 shadow-sm ${
                      isLatest ? 'ring-1 ring-teal-500/50 bg-teal-950/20 shadow-md shadow-teal-500/10' : ''
                    }`}
                  >
                    <div className="flex items-start justify-between gap-2">
                      <div className="flex items-center gap-1.5 flex-wrap">
                        ...
                        {isLatest && (
                          <span className="px-1.5 py-0.2 rounded text-[8px] font-bold bg-teal-500/20 text-teal-300 border border-teal-500/40 animate-pulse">
                            ✨ LATEST
                          </span>
                        )}
      ...
      <div className="px-4 py-2 border-t border-slate-800/80 bg-slate-900/60 flex items-center justify-between text-[10px] text-slate-400 font-mono shrink-0">
        <div className="flex items-center space-x-2">
          ...
          <span>Buffer: {displayLogs.length} events ({sortOrder === 'newest_top' ? 'Newest on Top' : 'Oldest on Top'})</span>
        </div>
        ...
        <div className="flex items-center space-x-2">
          <button
            ...
            className="px-2 py-0.5 rounded bg-slate-800 hover:bg-slate-700 text-teal-300 transition text-[10px] border border-slate-700 cursor-pointer"
            title="Scroll directly to the newest telemetry event"
          >
            {sortOrder === 'newest_top' ? '⬆️ Top (Latest)' : '⬇️ Bottom (Latest)'}
          </button>
