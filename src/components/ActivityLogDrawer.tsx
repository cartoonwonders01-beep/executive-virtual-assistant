  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 bg-slate-950/80 backdrop-blur-md animate-fadeIn">
      <div className="bg-slate-900 border border-slate-800 rounded-3xl w-full max-w-4xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden">
        ...
        {/* Filter & Hourly Summary Bar */}
        <div className="px-5 py-2.5 bg-slate-950/40 border-b border-slate-800/80 flex flex-wrap items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-3 text-[11px] text-slate-400 font-mono">
            <button
              onClick={() => {
                logger.curateHourlyLogs();
                setEntries(logger.getEntries());
              }}
              className="px-2.5 py-1 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-300 border border-slate-700 transition"
              title="Curate logs and archive entries older than 1 hour"
            >
              🔄 Curate Hourly Window
            </button>
            <span className="text-slate-500">Session: <strong className="text-slate-300">{logger.getSessionId().slice(0, 16)}</strong></span>
            <span className="text-slate-500">Total: <strong className="text-slate-300">{entries.length}</strong></span>
          </div>
        </div>
